<?php
/**
* This script creates a PDF confirmation page for Mission Net applicants
* OM - Germany
...
//#################Mission-Net ADDRESS#######
	...
	$pdf->SetX(20);
	$pdf->SetY(100);
	$pdf->Write(5, T_("The registration is only valid after we received this page and the payment has been made."));
	//Notfalladresse
	$pdf->SetY(110);
	$pdf->SetX(130);
...
//################REGISTRATION############
	...
	$pdf->SetFont('Arial','',12);
	$pdf->Ln(10);
	$pdf->Write(5, T_("With my signature and the signature of my parents (if participant is under 18 years) we ensure that we have made only valid statements in the registration process."));
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Write(5, T_("We have read the imprint and agree to them."));
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Write(5, T_("The imprint can be downloaded again by the following link:"));
	$pdf->Ln();
	//Link setzen.
	$pdf->Write(5, T_("www.mission-net.org/register/agb_en.pdf"), T_("www.mission-net.org/register/agb_en.pdf"));
	$pdf->ln(15);
	$pdf->Write(5, T_("Date and signature of participant") . ":   __________________________");
	// if younger than 18 print the following line
	if ( strtotime($values["dateofbirth"]) < strtotime("08.04.1991") ) {
	  $pdf->ln(15);
	  $pdf->Write(5, T_("Date and signature of parents") . ":       __________________________");
	}
//################SECOND PAGE FOR PAYMENT DETAILS################
	...
	$pdf->SetLeftMargin(20);
        ...
        $pdf->Cell(0,1, T_("Payment Instructions"),0,0,'C');
        $pdf->SetFont('Arial','',12);
        $pdf->SetY(70);
        $pdf->Write(5, T_("There are two way of paying for Mission-Net 2009:"));
        $pdf->Ln();
        $pdf->Write(5, T_(" - Wire transfer of money to our bank account"));
        $pdf->Ln();
        ...
        $pdf->Write(5, T_("to"));
        $pdf->SetX(100);
        $pdf->Write(5, T_("OM Europa / Mission-Net"));
        $pdf->Ln();
        $pdf->SetX(100);
        $pdf->Write(5, T_("Account no:") . " 5010802");
        $pdf->Ln();
        $pdf->SetX(100);
        $pdf->Write(5, $iban);
        $pdf->Ln();
        $pdf->SetX(100);
        $pdf->Write(5, T_("SWIFT CODE: GENODEF1EK1"));
        $pdf->Ln();
        $pdf->SetX(20);
        $pdf->Write(5, T_("Address of bank:"));
        $pdf->SetX(100);
        $pdf->Write(5, T_("Ev. Kreditgenossenschaft / Seidlerstr. 6 / D-34117 Kassel"));
        $pdf->Ln();
	...
	$plink = "https://register.mission-net.org/paypal.php?regi=" . $id;
        ...
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Write(5, T_("Note:") . " " . T_("Your registration is only valid as soon as we received your payment."));
        $pdf->Ln();
        $pdf->Ln();
